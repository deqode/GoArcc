// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.26.0
// 	protoc        v3.6.1
// source: git-service.proto

package pb

import (
	types "alfred/protos/types"
	timestamp "github.com/golang/protobuf/ptypes/timestamp"
	_ "github.com/ysugimoto/grpc-graphql-gateway/graphql"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type CloneRepositoryRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Provider      types.GitProviders `protobuf:"varint,1,opt,name=provider,proto3,enum=alfred.types.GitProviders" json:"provider,omitempty"`
	AccountId     string             `protobuf:"bytes,2,opt,name=account_id,json=accountId,proto3" json:"account_id,omitempty"`
	RepositoryUrl string             `protobuf:"bytes,3,opt,name=repository_url,json=repositoryUrl,proto3" json:"repository_url,omitempty"`
	BranchName    string             `protobuf:"bytes,4,opt,name=branch_name,json=branchName,proto3" json:"branch_name,omitempty"`
	UserName      string             `protobuf:"bytes,5,opt,name=user_name,json=userName,proto3" json:"user_name,omitempty"`
}

func (x *CloneRepositoryRequest) Reset() {
	*x = CloneRepositoryRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_git_service_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CloneRepositoryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloneRepositoryRequest) ProtoMessage() {}

func (x *CloneRepositoryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_git_service_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloneRepositoryRequest.ProtoReflect.Descriptor instead.
func (*CloneRepositoryRequest) Descriptor() ([]byte, []int) {
	return file_git_service_proto_rawDescGZIP(), []int{0}
}

func (x *CloneRepositoryRequest) GetProvider() types.GitProviders {
	if x != nil {
		return x.Provider
	}
	return types.GitProviders_UNKNOWN
}

func (x *CloneRepositoryRequest) GetAccountId() string {
	if x != nil {
		return x.AccountId
	}
	return ""
}

func (x *CloneRepositoryRequest) GetRepositoryUrl() string {
	if x != nil {
		return x.RepositoryUrl
	}
	return ""
}

func (x *CloneRepositoryRequest) GetBranchName() string {
	if x != nil {
		return x.BranchName
	}
	return ""
}

func (x *CloneRepositoryRequest) GetUserName() string {
	if x != nil {
		return x.UserName
	}
	return ""
}

type CloneRepositoryResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	WorkflowId string `protobuf:"bytes,1,opt,name=workflow_id,json=workflowId,proto3" json:"workflow_id,omitempty"`
	RunId      string `protobuf:"bytes,2,opt,name=run_id,json=runId,proto3" json:"run_id,omitempty"`
}

func (x *CloneRepositoryResponse) Reset() {
	*x = CloneRepositoryResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_git_service_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CloneRepositoryResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloneRepositoryResponse) ProtoMessage() {}

func (x *CloneRepositoryResponse) ProtoReflect() protoreflect.Message {
	mi := &file_git_service_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloneRepositoryResponse.ProtoReflect.Descriptor instead.
func (*CloneRepositoryResponse) Descriptor() ([]byte, []int) {
	return file_git_service_proto_rawDescGZIP(), []int{1}
}

func (x *CloneRepositoryResponse) GetWorkflowId() string {
	if x != nil {
		return x.WorkflowId
	}
	return ""
}

func (x *CloneRepositoryResponse) GetRunId() string {
	if x != nil {
		return x.RunId
	}
	return ""
}

//todo: rename git url to generic name
type Repository struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id            int64                `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	NodeId        string               `protobuf:"bytes,2,opt,name=node_id,json=nodeId,proto3" json:"node_id,omitempty"`
	Name          string               `protobuf:"bytes,3,opt,name=name,proto3" json:"name,omitempty"`
	FullName      string               `protobuf:"bytes,4,opt,name=full_name,json=fullName,proto3" json:"full_name,omitempty"`
	DefaultBranch string               `protobuf:"bytes,6,opt,name=default_branch,json=defaultBranch,proto3" json:"default_branch,omitempty"`
	MasterBranch  string               `protobuf:"bytes,7,opt,name=master_branch,json=masterBranch,proto3" json:"master_branch,omitempty"`
	CreatedAt     *timestamp.Timestamp `protobuf:"bytes,8,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	PushedAt      *timestamp.Timestamp `protobuf:"bytes,9,opt,name=pushed_at,json=pushedAt,proto3" json:"pushed_at,omitempty"`
	UpdatedAt     *timestamp.Timestamp `protobuf:"bytes,10,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	CloneUrl      string               `protobuf:"bytes,11,opt,name=clone_url,json=cloneUrl,proto3" json:"clone_url,omitempty"`
	GitUrl        string               `protobuf:"bytes,12,opt,name=git_url,json=gitUrl,proto3" json:"git_url,omitempty"`
	Size          int32                `protobuf:"varint,13,opt,name=size,proto3" json:"size,omitempty"`
	Private       bool                 `protobuf:"varint,14,opt,name=private,proto3" json:"private,omitempty"`
	Branches      []string             `protobuf:"bytes,15,rep,name=branches,proto3" json:"branches,omitempty"`
}

func (x *Repository) Reset() {
	*x = Repository{}
	if protoimpl.UnsafeEnabled {
		mi := &file_git_service_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Repository) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Repository) ProtoMessage() {}

func (x *Repository) ProtoReflect() protoreflect.Message {
	mi := &file_git_service_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Repository.ProtoReflect.Descriptor instead.
func (*Repository) Descriptor() ([]byte, []int) {
	return file_git_service_proto_rawDescGZIP(), []int{2}
}

func (x *Repository) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *Repository) GetNodeId() string {
	if x != nil {
		return x.NodeId
	}
	return ""
}

func (x *Repository) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Repository) GetFullName() string {
	if x != nil {
		return x.FullName
	}
	return ""
}

func (x *Repository) GetDefaultBranch() string {
	if x != nil {
		return x.DefaultBranch
	}
	return ""
}

func (x *Repository) GetMasterBranch() string {
	if x != nil {
		return x.MasterBranch
	}
	return ""
}

func (x *Repository) GetCreatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *Repository) GetPushedAt() *timestamp.Timestamp {
	if x != nil {
		return x.PushedAt
	}
	return nil
}

func (x *Repository) GetUpdatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.UpdatedAt
	}
	return nil
}

func (x *Repository) GetCloneUrl() string {
	if x != nil {
		return x.CloneUrl
	}
	return ""
}

func (x *Repository) GetGitUrl() string {
	if x != nil {
		return x.GitUrl
	}
	return ""
}

func (x *Repository) GetSize() int32 {
	if x != nil {
		return x.Size
	}
	return 0
}

func (x *Repository) GetPrivate() bool {
	if x != nil {
		return x.Private
	}
	return false
}

func (x *Repository) GetBranches() []string {
	if x != nil {
		return x.Branches
	}
	return nil
}

type ListRepositoryRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Provider  types.GitProviders `protobuf:"varint,1,opt,name=provider,proto3,enum=alfred.types.GitProviders" json:"provider,omitempty"`
	UserId    string             `protobuf:"bytes,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	AccountId string             `protobuf:"bytes,3,opt,name=account_id,json=accountId,proto3" json:"account_id,omitempty"`
	Label     string             `protobuf:"bytes,4,opt,name=label,proto3" json:"label,omitempty"`
}

func (x *ListRepositoryRequest) Reset() {
	*x = ListRepositoryRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_git_service_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListRepositoryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRepositoryRequest) ProtoMessage() {}

func (x *ListRepositoryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_git_service_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRepositoryRequest.ProtoReflect.Descriptor instead.
func (*ListRepositoryRequest) Descriptor() ([]byte, []int) {
	return file_git_service_proto_rawDescGZIP(), []int{3}
}

func (x *ListRepositoryRequest) GetProvider() types.GitProviders {
	if x != nil {
		return x.Provider
	}
	return types.GitProviders_UNKNOWN
}

func (x *ListRepositoryRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *ListRepositoryRequest) GetAccountId() string {
	if x != nil {
		return x.AccountId
	}
	return ""
}

func (x *ListRepositoryRequest) GetLabel() string {
	if x != nil {
		return x.Label
	}
	return ""
}

type ListRepositoryResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Repositories []*Repository `protobuf:"bytes,1,rep,name=repositories,proto3" json:"repositories,omitempty"`
}

func (x *ListRepositoryResponse) Reset() {
	*x = ListRepositoryResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_git_service_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListRepositoryResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRepositoryResponse) ProtoMessage() {}

func (x *ListRepositoryResponse) ProtoReflect() protoreflect.Message {
	mi := &file_git_service_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRepositoryResponse.ProtoReflect.Descriptor instead.
func (*ListRepositoryResponse) Descriptor() ([]byte, []int) {
	return file_git_service_proto_rawDescGZIP(), []int{4}
}

func (x *ListRepositoryResponse) GetRepositories() []*Repository {
	if x != nil {
		return x.Repositories
	}
	return nil
}

type GetRepositoryRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Provider  types.GitProviders `protobuf:"varint,1,opt,name=provider,proto3,enum=alfred.types.GitProviders" json:"provider,omitempty"`
	RepoName  string             `protobuf:"bytes,2,opt,name=repo_name,json=repoName,proto3" json:"repo_name,omitempty"`
	AccountId string             `protobuf:"bytes,3,opt,name=account_id,json=accountId,proto3" json:"account_id,omitempty"`
	OwnerName string             `protobuf:"bytes,4,opt,name=owner_name,json=ownerName,proto3" json:"owner_name,omitempty"`
}

func (x *GetRepositoryRequest) Reset() {
	*x = GetRepositoryRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_git_service_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetRepositoryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRepositoryRequest) ProtoMessage() {}

func (x *GetRepositoryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_git_service_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRepositoryRequest.ProtoReflect.Descriptor instead.
func (*GetRepositoryRequest) Descriptor() ([]byte, []int) {
	return file_git_service_proto_rawDescGZIP(), []int{5}
}

func (x *GetRepositoryRequest) GetProvider() types.GitProviders {
	if x != nil {
		return x.Provider
	}
	return types.GitProviders_UNKNOWN
}

func (x *GetRepositoryRequest) GetRepoName() string {
	if x != nil {
		return x.RepoName
	}
	return ""
}

func (x *GetRepositoryRequest) GetAccountId() string {
	if x != nil {
		return x.AccountId
	}
	return ""
}

func (x *GetRepositoryRequest) GetOwnerName() string {
	if x != nil {
		return x.OwnerName
	}
	return ""
}

type GetCloningStatusRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	WorkflowId string `protobuf:"bytes,1,opt,name=workflow_id,json=workflowId,proto3" json:"workflow_id,omitempty"`
	RunId      string `protobuf:"bytes,2,opt,name=run_id,json=runId,proto3" json:"run_id,omitempty"`
}

func (x *GetCloningStatusRequest) Reset() {
	*x = GetCloningStatusRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_git_service_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetCloningStatusRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetCloningStatusRequest) ProtoMessage() {}

func (x *GetCloningStatusRequest) ProtoReflect() protoreflect.Message {
	mi := &file_git_service_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetCloningStatusRequest.ProtoReflect.Descriptor instead.
func (*GetCloningStatusRequest) Descriptor() ([]byte, []int) {
	return file_git_service_proto_rawDescGZIP(), []int{6}
}

func (x *GetCloningStatusRequest) GetWorkflowId() string {
	if x != nil {
		return x.WorkflowId
	}
	return ""
}

func (x *GetCloningStatusRequest) GetRunId() string {
	if x != nil {
		return x.RunId
	}
	return ""
}

type GetCloningStatusResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Status bool `protobuf:"varint,1,opt,name=status,proto3" json:"status,omitempty"`
}

func (x *GetCloningStatusResponse) Reset() {
	*x = GetCloningStatusResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_git_service_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetCloningStatusResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetCloningStatusResponse) ProtoMessage() {}

func (x *GetCloningStatusResponse) ProtoReflect() protoreflect.Message {
	mi := &file_git_service_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetCloningStatusResponse.ProtoReflect.Descriptor instead.
func (*GetCloningStatusResponse) Descriptor() ([]byte, []int) {
	return file_git_service_proto_rawDescGZIP(), []int{7}
}

func (x *GetCloningStatusResponse) GetStatus() bool {
	if x != nil {
		return x.Status
	}
	return false
}

var File_git_service_proto protoreflect.FileDescriptor

var file_git_service_proto_rawDesc = []byte{
	,
}

var (
	file_git_service_proto_rawDescOnce sync.Once
	file_git_service_proto_rawDescData = file_git_service_proto_rawDesc
)

func file_git_service_proto_rawDescGZIP() []byte {
	file_git_service_proto_rawDescOnce.Do(func() {
		file_git_service_proto_rawDescData = protoimpl.X.CompressGZIP(file_git_service_proto_rawDescData)
	})
	return file_git_service_proto_rawDescData
}

var file_git_service_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_git_service_proto_goTypes = []interface{}{
	(*CloneRepositoryRequest)(nil),   // 0: alfred.git.v1.CloneRepositoryRequest
	(*CloneRepositoryResponse)(nil),  // 1: alfred.git.v1.CloneRepositoryResponse
	(*Repository)(nil),               // 2: alfred.git.v1.Repository
	(*ListRepositoryRequest)(nil),    // 3: alfred.git.v1.ListRepositoryRequest
	(*ListRepositoryResponse)(nil),   // 4: alfred.git.v1.ListRepositoryResponse
	(*GetRepositoryRequest)(nil),     // 5: alfred.git.v1.GetRepositoryRequest
	(*GetCloningStatusRequest)(nil),  // 6: alfred.git.v1.GetCloningStatusRequest
	(*GetCloningStatusResponse)(nil), // 7: alfred.git.v1.GetCloningStatusResponse
	(types.GitProviders)(0),          // 8: alfred.types.GitProviders
	(*timestamp.Timestamp)(nil),      // 9: google.protobuf.Timestamp
}
var file_git_service_proto_depIdxs = []int32{
	8,  // 0: alfred.git.v1.CloneRepositoryRequest.provider:type_name -> alfred.types.GitProviders
	9,  // 1: alfred.git.v1.Repository.created_at:type_name -> google.protobuf.Timestamp
	9,  // 2: alfred.git.v1.Repository.pushed_at:type_name -> google.protobuf.Timestamp
	9,  // 3: alfred.git.v1.Repository.updated_at:type_name -> google.protobuf.Timestamp
	8,  // 4: alfred.git.v1.ListRepositoryRequest.provider:type_name -> alfred.types.GitProviders
	2,  // 5: alfred.git.v1.ListRepositoryResponse.repositories:type_name -> alfred.git.v1.Repository
	8,  // 6: alfred.git.v1.GetRepositoryRequest.provider:type_name -> alfred.types.GitProviders
	3,  // 7: alfred.git.v1.git.listRepository:input_type -> alfred.git.v1.ListRepositoryRequest
	5,  // 8: alfred.git.v1.git.GetRepository:input_type -> alfred.git.v1.GetRepositoryRequest
	0,  // 9: alfred.git.v1.git.CloneRepository:input_type -> alfred.git.v1.CloneRepositoryRequest
	6,  // 10: alfred.git.v1.git.GetCloningStatus:input_type -> alfred.git.v1.GetCloningStatusRequest
	4,  // 11: alfred.git.v1.git.listRepository:output_type -> alfred.git.v1.ListRepositoryResponse
	2,  // 12: alfred.git.v1.git.GetRepository:output_type -> alfred.git.v1.Repository
	1,  // 13: alfred.git.v1.git.CloneRepository:output_type -> alfred.git.v1.CloneRepositoryResponse
	7,  // 14: alfred.git.v1.git.GetCloningStatus:output_type -> alfred.git.v1.GetCloningStatusResponse
	11, // [11:15] is the sub-list for method output_type
	7,  // [7:11] is the sub-list for method input_type
	7,  // [7:7] is the sub-list for extension type_name
	7,  // [7:7] is the sub-list for extension extendee
	0,  // [0:7] is the sub-list for field type_name
}

func init() { file_git_service_proto_init() }
func file_git_service_proto_init() {
	if File_git_service_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_git_service_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CloneRepositoryRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_git_service_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CloneRepositoryResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_git_service_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Repository); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_git_service_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListRepositoryRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_git_service_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListRepositoryResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_git_service_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetRepositoryRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_git_service_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetCloningStatusRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_git_service_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetCloningStatusResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_git_service_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_git_service_proto_goTypes,
		DependencyIndexes: file_git_service_proto_depIdxs,
		MessageInfos:      file_git_service_proto_msgTypes,
	}.Build()
	File_git_service_proto = out.File
	file_git_service_proto_rawDesc = nil
	file_git_service_proto_goTypes = nil
	file_git_service_proto_depIdxs = nil
}
